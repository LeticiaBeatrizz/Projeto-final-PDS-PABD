...
class AdicionarEstoqueMixin:
    """Modal de adição de unidades ao estoque de um álbum.
    ...
    def abrir_modal_adicionar_estoque(self):
        albuns = self.obter_albuns()
        if not albuns:
            messagebox.showwarning("Aviso", "Cadastre um álbum antes de adicionar estoque.")
            return

        modal, frame_modal, botoes_frame = self.criar_modal_rolavel("Adicionar ao estoque", largura=500, altura_desejada=500)

        titulo = tk.Label(frame_modal, text="Adicionar ao estoque", font=("poppins", 16, "bold"), fg="#5f1c16", bg="#ffffff")
        ...
        def atualizar_previa(*_args):
            ...
            lbl_apos_operacao.config(text=f"Após a operação: '{atual + quantidade}'")
        ...
        def confirmar_adicao():
            ...
            try:
                self.servico_estoque.adicionar_multiplos(album_selecionado["id"], quantidade)
            except ValueError as erro:
                lbl_mensagem_modal.config(text=str(erro))
                return
            except Exception:
                lbl_mensagem_modal.config(text="Falha ao adicionar ao estoque no banco de dados.")
                return

            registrar_atividade(f"Estoque do álbum '{album_selecionado['nome']}' aumentado em {quantidade} unidade(s).")
            messagebox.showinfo("Sucesso", "Estoque atualizado com sucesso!")
            modal.destroy()
            self.atualizar_listagem(self.obter_albuns())

        canvas_btn_adicionar = self.desenhar_fundo_arredondado(botoes_frame, largura=190, altura=42, raio=18, cor_fundo="#ffffff", cor_forma="#d14d3a")
        canvas_btn_adicionar.pack(side="left", padx=(8, 0))
        btn_adicionar = tk.Button(canvas_btn_adicionar, text="Adicionar", font=("poppins", 10, "bold"), fg="#ffffff", bg="#d14d3a", bd=0, activebackground="#b63e2a", cursor="hand2", command=confirmar_adicao)
        canvas_btn_adicionar.create_window(95, 21, window=btn_adicionar, width=170, height=38)
